...
- **Animated image support** — detects animated GIF (multiple image descriptor blocks) and animated WebP (ANIM/ANMF chunks in RIFF container). Animated GIFs under size limit pass through directly. Animated WebPs are frame-extracted and re-encoded as GIF (server only detects animation for `image/gif` content type). Frame extraction via `android.graphics.Movie` (GIF, all API levels) and `ImageDecoder`/`AnimatedImageDrawable` (WebP, API 28+, using coroutine `delay()` for frame stepping). Crop applied per-frame using normalized coordinates. Loop detection compares against first-frame hash to avoid premature truncation on animations with repeated mid-sequence frames
- **GIF89a encoder** (`GifEncoder.kt`) — self-contained animated GIF encoder with LZW compression, median-cut color quantization (256 colors), per-frame Local Color Tables (ensures correct colors when frames have different palettes), NETSCAPE 2.0 looping extension, transparency support, and configurable per-frame delay/disposal
...
- **CSS colour/gradient parsing hardening** — `BrushCompat` and `TextViewCompat` CSS role-colour parsers could crash on malformed input: `toFloat()` on non-numeric stop values, division by zero with single-stop gradients, and Compose `linearGradient()`/`radialGradient()` crashing on empty colorStops arrays. Fixed with `toFloatOrNull()` fallbacks, `coerceAtLeast(1)` for denominators, empty-stops guards returning solid colour fallback, and top-level try-catch on non-composable paths. Covers both composable `BrushCompat` and non-composable `InstancedBrushCompat`/`TextViewCompat`.
- **Null-safety audit batch 2 (6 files)** — replaced ~30 more force-unwraps with null-safe patterns: RealtimeSocket ready frame cache population (filter null IDs before `associateBy`, local `val` captures for smart cast in server/channel/emoji DB upserts and WS frame handlers), StoatAPI local DB hydration (`mapNotNull` for channel IDs), Message content rendering (named lambda param instead of `!!` inside `?.let`), Embed URL access (safe casts via `?.let`), Autocomplete emoji shortcodes (`.firstOrNull()`), LoginScreen session hints (guard `firstUserHints` null with error message instead of crash).
- **Unsafe cast elimination (4 files)** — replaced redundant `as` casts with local `val` captures for Kotlin smart cast: ImageProcessor bitmap processing chain (`var` bitmaps can't smart-cast, now use local `val` with null guard), ChatRouterScreen `currentDestination` sealed class dispatch, ProfileSettingsScreen URI model dispatch, FriendsScreen QR result dispatch.
