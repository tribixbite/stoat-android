...
| `/perms` | Manage Channels | Check bot permissions on linked Stoat server, show available/missing capabilities |
...
4. `force_reauth` — Force re-authorization even if guild is already linked (default: reuse existing link)
...
| `!stoatcord perms` | No | Check bot permissions and show available/missing capabilities |
...
Initial link to a Stoat server requires dual-admin auth. Subsequent `/migrate` runs on already-linked guilds skip re-auth by reusing the stored `server_links` entry. Use `force_reauth:true` to require fresh authorization (e.g. to re-link to a different server).

**Four auth paths for `/migrate` (checked in order):**

| Path | Trigger | Auth |
|------|---------|------|
| **0: Existing link** | Guild already in `server_links`, no code/ID provided | None — reuses stored link (skip re-auth) |
| **A: New server** | No code, no server ID, no existing link | None — bot creates and owns it |
